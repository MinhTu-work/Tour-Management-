...
import com.easy.tour.Tour_View.dto.ModifiedRoleDTO;
...
import com.easy.tour.Tour_View.response.ModifiedRoleResponseDTO;
...
public class ProfileUserController {
    ...
    public String viewProfilePage(Model model,
        ...
        // Data
        List<UserDTO> userDtoList = response.getList();
        if (userDtoList != null) {
            for (UserDTO userDTO : userDtoList) {
                Set<String> roles = userDTO.getRoles();
                if (roles != null && !roles.isEmpty()) {
                    userDTO.setRole(roles.iterator().next());
                }
            }
        }
    ...
    @GetMapping(value = UrlPath.PROFILE_MODIFIED_ROLE)
    public String modifiedRolePage(@RequestParam("uuid") String uuid,
                                   Model model,
                                   HttpServletRequest request) {
        // UI
        model.addAttribute("activeNav", "profile");
        ...
        ModifiedRoleResponseDTO response = restTemplateUtils.getData(ApiPath.USER_GET_UUID + uuid, request, ModifiedRoleResponseDTO.class);

        ModifiedRoleDTO modifiedRoleDTO = response.getData();
        model.addAttribute("modifiedRoleDTO", modifiedRoleDTO);
        return "profile/profileModifiedRole";
    }

    @PostMapping(value = UrlPath.PROFILE_MODIFIED_ROLE, params = "action")
    public String modifiedRoleSubmit(@RequestParam(value="action", required = true) String action,
                                     @RequestParam("uuid") String uuid,
                                     Model model,
                                     HttpServletRequest request,
                                     @Valid @ModelAttribute("modifiedRoleDTO") ModifiedRoleDTO modifiedRoleDTO,
                                     BindingResult result
                                     ) {
        // Cancel
        if ("cancel".equals(action)) {}
...
//    @GetMapping(value = UrlPath.PROFILE_DELETE_USER)
//    public String deleteUser(@RequestParam("uuid") String uuid,
//                             HttpServletRequest request) {
//        UserResponseDTO response = restTemplateUtils.deleteData(uuid, ApiPath.USER_DELETE + uuid, request, UserResponseDTO.class);
//        return "redirect:" + UrlPath.PROFILE_VIEW_ALL_PAGE;
//    }
